...
function houseLog(env::environment,haus::newHouse)
    dArray=DataFrame([[env.key],[env.tick],[haus.index],[haus.quality]])
...
function agtLog(env::environment,agt::agent)
    dArray=DataFrame([[env.key],[env.tick],[agt.init],[agt.budget]])
...
function hotelGenLog(env::environment,hot::hotel)
    dArray=DataFrame([[env.key],[env.tick],[hot.index],[hot.quality],[hot.owner.init]])
...
function loanLog(env::environment,ln::loan)
    dArray=DataFrame([[env.key],[env.tick],[ln.interestRate],[ln.initialBalance],[ln.monthlyPayment],[ln.collateral.index]])
...
function loanFullLog(env::environment,ln::loan)
    dArray=DataFrame([[env.key],[env.tick],[ln.interestRate],[ln.collateral.index]])
...
function agtMoveIn(env::environment,dwell::dwelling,agt::agent)
    dArray=DataFrame([[env.key],[env.tick],[dwell.index],[agt.index]])
...
function agtLeave(env::environment,origin::dwelling,agt::agent)
    dArray=DataFrame([[env.key],[env.tick],[origin.index],[agt.index]])
...
function agtMove(env::environment,origin::dwelling,dest::dwelling,agt::agent)
    dArray=DataFrame([[env.key],[env.tick],[origin.index],[dest.index],[agt.index]])
